...
X2_quantile25 <- quantile(X$X2)[3]
X3_quantile25 <- quantile(X$X3)[3]
y_quantile50 <- quantile(y)[3]

# 75%
X1_quantile75 <- quantile(X$X1)[4]
X2_quantile25 <- quantile(X$X2)[4]
X3_quantile25 <- quantile(X$X3)[4]
...
test_that("Testing EDA for multi continuous feature", {
  ...
  expect_match(colnames(summary), c('mean', 'variance', 'quantile25','quantile50','quantile75')) 
  ...
  expect_equal(summary$quantile25[3], X1_quantile25)
  expect_equal(summary$quantile25[4], X1_quantile25)
